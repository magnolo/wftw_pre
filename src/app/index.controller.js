...
angular.module('preSite')
	.controller('LogoCtrl', function ($scope, $interval) {
		$scope.logo = {
			size: 150,
			width: 5
		};
		...
		$scope.logo1options = {
			size: $scope.logo.size,
			width: $scope.logo.width
		};
		$scope.logo2options = {
			size: $scope.logo.size,
			width: $scope.logo.width,
		...
		$scope.toggleValues = function () {
			$scope.percent = Math.floor((Math.random() * 100) + 1);
			$scope.percent2 = Math.floor((Math.random() * 100) + 1);
			$scope.percent3 = Math.floor((Math.random() * 100) + 1);
		};
		$interval(function () {
			$scope.toggleValues();
		}, 2500);
